...
You are generating an implementation plan for the **sbam** project (Smart Battery Advanced Manager — see [CLAUDE.md](../../CLAUDE.md)). The user invokes this prompt as:
...
2. **If it exists:**
   ...
   - Ask **only the necessary clarifying questions** (use the questions tool; batch them; prefer fixed-choice options when possible). Do not ask questions whose answers are already in the TASK or in `CLAUDE.md`.
...
1. **Codebase analysis** — find and cite existing patterns to mirror. Consult [CLAUDE.md](../../CLAUDE.md) (Project Structure section) for file locations, and read the specific files relevant to the feature.
