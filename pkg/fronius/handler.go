...
func (fronius *Fronius) Handler(pw_forecast float64, pw_batt2charge float64, pw_batt_max float64, pw_consumption float64, max_charge float64, pw_batt_reserve float64, start_hr string, end_hr string, fronius_ip string, batt_reserve_charge_enabled bool, pw_lwt float64, pw_upt float64, forecast_charge_enabled bool, fronius_port ...string) (int16, Decision, string, PowerState, error) {
	...
	charge_pc, decision, reason, pw, err := SetFroniusChargeBatteryMode(pw_forecast, pw_batt2charge, pw_batt_max, pw_consumption, max_charge, pw_batt_reserve, start_hr, end_hr, fronius_ip, batt_reserve_charge_enabled, pw_lwt, pw_upt, forecast_charge_enabled, p)
	if err != nil {
		u.Log.Errorln("Error setting Fronius Battery charge: %s ", err)
		return charge_pc, decision, reason, pw, err
	}

	return charge_pc, decision, reason, pw, nil
