...
func BuildDiscovery(cfg Config, version string) []DiscoveryEntity {
	...
	entities = appendDiscoveryEntity(entities, discoveryPrefix, "sensor", "pw_net_wh", sensorPayload(base, deviceID, "pw_net_wh", "Net Energy", "{% if value_json.pw_net_wh is defined and value_json.pw_net_wh is not none %}{{ '%.2f' | format(value_json.pw_net_wh | float) }}{% endif %}", "Wh", "", "measurement", "diagnostic"))
